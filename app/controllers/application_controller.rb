class ApplicationController < ActionController::Base
  ...
  def check_status(status)
    redirect_to send("event_#{@event.status}_path"), warning: "Prohibited action!" unless @event.status == status
  end
  ...
  def authorize_event
    redirect_to root_path, alert: "It's not your event!" unless event_owner?
  end
